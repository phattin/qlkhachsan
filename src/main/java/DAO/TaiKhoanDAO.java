...
public class TaiKhoanDAO {
    ...
    public String increaseMaTK() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                maTK = rs.getString("MaxMaTK");
                if (maTK != null) {
                    int newMaTK = Integer.parseInt(maTK.substring(2)) + 1;
                    maTK = "TK" + String.format("%03d", newMaTK);
                } else {
                    maTK = "TK001"; // Nếu không có nhân viên nào, bắt đầu từ NV001
                }
            }
    ...
    public boolean add(TaiKhoanDTO tkDTO) {
        int result = 0;
        String sql = "INSERT INTO taikhoan (MaTK, TenDangNhap, MatKhau, MaNhanVien, MaChucVu, TrangThai) VALUES (?, ?, ?, ?, ?, ?)";

        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {

            pstmt.setString(1, increaseMaTK());
            pstmt.setString(2, tkDTO.getTenDangNhap());
            pstmt.setString(3, tkDTO.getMatKhau());   
            pstmt.setString(4, tkDTO.getMaNhanVien());   
            pstmt.setString(5, tkDTO.getMaChucVu());   
            pstmt.setString(6, tkDTO.getTrangThai());
