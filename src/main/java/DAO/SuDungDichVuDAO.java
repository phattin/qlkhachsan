...
public class SuDungDichVuDAO {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql);
            ...
            while (rs.next()) {
                list.add(new SuDungDichVuDTO(
                    String.valueOf(rs.getString("MaSuDungDV")), // Chuyển thành String
    ...
    // Lấy danh sách tất cả các dịch vụ đã sử dụng theo mã hóa đơn
    public ArrayList<SuDungDichVuDTO> getDanhSachDichVuTheoMaHoaDon(String maHD) {
        ArrayList<SuDungDichVuDTO> list = new ArrayList<>();
        String sql = "SELECT * FROM sudungdichvu sddv JOIN datphong dp ON sddv.MaDatPhong = dp.MaDatPhong JOIN hoadon hd ON hd.MaDatPhong = dp.MaDatPhong WHERE hd.MaHoaDon = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, maHD);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new SuDungDichVuDTO(
                    String.valueOf(rs.getString("sddv.MaSuDungDV")), // Chuyển thành String
                    rs.getString("sddv.MaDatPhong"),
                    rs.getString("sddv.MaDV"),
                    rs.getInt("sddv.SoLuong")
                ));
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public SuDungDichVuDTO getSuDungDichVuByMa(String MaSuDungDV) {
        String sql = "SELECT * FROM sudungdichvu WHERE MaSuDungDV = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, MaSuDungDV);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                return new SuDungDichVuDTO(
                    String.valueOf(rs.getInt("MaSuDungDV")), // Chuyển thành String
                    rs.getString("MaDatPhong"),
                    rs.getString("MaDV"),
                    rs.getInt("SoLuong")
                );
