...
public class DanhSachPhongDAO {
    ...
    // Lấy danh sách tất cả các phòng đã đặt theo mã hóa đơn
    public ArrayList<DanhSachPhongDTO> getDanhSachPhongTheoMaHoaDon(String maHD) {
        ArrayList<DanhSachPhongDTO> list = new ArrayList<>();
        String sql = "SELECT * FROM danhsachphong dsp JOIN datphong dp ON  dsp.MaDatPhong = dp.MaDatPhong JOIN hoadon hd ON hd.MaDatPhong = dp.MaDatPhong WHERE hd.MaHoaDon = ?";
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, maHD);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                list.add(new DanhSachPhongDTO(
                    rs.getString("dsp.MaDSP"),
                    rs.getString("dsp.MaDatPhong"),
                    rs.getString("dsp.MaPhong")
                ));
            }
        } catch (Exception e) {
            e.printStackTrace();
