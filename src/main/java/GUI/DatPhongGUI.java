...
public class DatPhongGUI extends JPanel {
    ...
    private void themDatPhong() {
        ...
        JTextField txtMaDatPhong = new JTextField(datPhongBUS.increaseMaDatPhong());
        ...
        btnThemVaoPhieu.addActionListener(e -> {
            themDichVuVaoPhieu(dialog, dichVuTable, dichVuTableModel, txtSoLuong, 
                              suDungDichVuTableModel, txtTongTienDichVu, txtTongTienPhong, 
                              txtTongTienDatPhong, txtMaDatPhong);
        });
            
        // Cập nhật xử lý cho nút xóa dịch vụ
        xoaDV.addActionListener(e -> {
            xoaDichVu(dialog, suDungDichVuTable, suDungDichVuTableModel, txtTongTienDichVu, 
                     txtTongTienPhong, txtTongTienDatPhong);
        });

        // Thêm xử lý cho nút sửa số lượng
        suaDV.addActionListener(e -> {
            suaSoLuongDichVu(dialog, suDungDichVuTable, suDungDichVuTableModel, dichVuTableModel,
                            txtTongTienDichVu, txtTongTienPhong, txtTongTienDatPhong);
        });


        btnThemPhong.addActionListener(e -> {
            themPhong(dialog, phongTable, phongTableModel, phongDatTableModel, spinnerNgayNhan, 
                     spinnerNgayTra, txtTongTienPhong, txtTongTienDichVu, txtTongTienDatPhong, 
                     txtMaDatPhong);
        });

        // Xử lý sự kiện khi nhấn nút xóa phòng đã đặt
        xoaPhongDat.addActionListener(e -> {
            xoaPhongDaDat(dialog, phongDatTable, phongDatTableModel, phongTableModel, 
                         txtTongTienPhong, txtTongTienDichVu, txtTongTienDatPhong);
        });

        btnXacNhan.addActionListener(e -> {
            xacNhanDatPhong(dialog, txtMaDatPhong, txtSDT, phongDatTableModel, 
                           spinnerNgayNhan, spinnerNgayTra, suDungDichVuTableModel, 
                           txtTongTienDatPhong);
        });
        // Hiển thị dialog
    ...
    private void xoaDatPhong() {
        ...
        if (confirm == JOptionPane.YES_OPTION) {
            try {
                ...
                System.out.println("Danh sách mã phòng cần cập nhật: " + danhSachMaPhong);
                ...
                // 5. Xóa mềm đặt phòng (chỉ cập nhật trường trangThaiXoa = 1)
                boolean xoaMemThanhCong = datPhongBUS.xoaMemDatPhong(maDatPhong); // bạn cần cài đặt hàm này

                if (xoaMemThanhCong) {
                    JOptionPane.showMessageDialog(this, 
                        "Xóa mềm đặt phòng thành công và tất cả phòng đã được chuyển về trạng thái Trống!", 
                        "Thông báo", JOptionPane.INFORMATION_MESSAGE);
                    loadTableData();

                } else {
                    JOptionPane.showMessageDialog(this, "Xóa mềm đặt phòng thất bại!", 
                        "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
            } catch (Exception ex) {
                JOptionPane.showMessageDialog(this, "Lỗi khi xóa mềm đặt phòng: " + ex.getMessage(), 
                    "Lỗi", JOptionPane.ERROR_MESSAGE);
                ex.printStackTrace();
            }
        }
    ...
    private void suaDatPhong() {
        ...
        btnCapNhat.addActionListener(e -> {
            capNhatDatPhong(dialog, txtMaDatPhong, txtSDT, phongDatTableModel,
                           spinnerNgayNhan, spinnerNgayTra, suDungDichVuTableModel,
                           datPhongHienTai, txtTongTienDatPhong);
        });
    ...
    /**
     * Phương thức xử lý khi nhấn nút xác nhận đặt phòng
     */
    private void xacNhanDatPhong(JDialog dialog, 
                            JTextField txtMaDatPhong, 
                            JTextField txtSDT,
                            DefaultTableModel phongDatTableModel, 
                            JSpinner spinnerNgayNhan, 
                            JSpinner spinnerNgayTra,
                            DefaultTableModel suDungDichVuTableModel,
                            JTextField txtTongTienDatPhong) {
        try {
            // Lấy thông tin từ form
            String maDatPhong = txtMaDatPhong.getText().trim();
            if (maDatPhong.isEmpty()) {
                JOptionPane.showMessageDialog(dialog, "Mã đặt phòng không được để trống!", 
                    "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                return;
            }
            
            String maKhachHang = (String) txtSDT.getClientProperty("MaKH");
            ...
            // Lấy thông tin ngày nhận và ngày trả
            java.util.Date ngayNhanUtil = (java.util.Date) spinnerNgayNhan.getValue();
            java.util.Date ngayTraUtil = (java.util.Date) spinnerNgayTra.getValue();
            
            if (ngayTraUtil.before(ngayNhanUtil) || ngayTraUtil.equals(ngayNhanUtil)) {
                JOptionPane.showMessageDialog(dialog, "Ngày trả phải sau ngày nhận!", 
                    "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                return;
            }
            
            Date ngayNhan = new Date(ngayNhanUtil.getTime());
            Date ngayTra = new Date(ngayTraUtil.getTime());
            
            // Chọn phòng đầu tiên làm phòng chính cho đặt phòng
            String maPhongDauTien = (String) phongDatTableModel.getValueAt(0, 0);
            
            // Tạo đối tượng DatPhongDTO
            DatPhongDTO datPhong = new DatPhongDTO(maDatPhong, maPhongDauTien, maKhachHang, ngayNhan, ngayTra, 0);
            
            // Thêm đặt phòng vào cơ sở dữ liệu
            boolean datPhongThanhCong = datPhongBUS.addDatPhong(datPhong);
            
            if (datPhongThanhCong) {
                // Lưu tất cả các phòng đã đặt vào danh sách phòng
                DanhSachPhongBUS danhSachPhongBUS = new DanhSachPhongBUS();
                ...
                // Lưu dịch vụ đã sử dụng vào cơ sở dữ liệu
                ...
                for (int i = 0; i < suDungDichVuTableModel.getRowCount(); i++) {
                    ...
                    String maDV = "";
                    DichVuBUS dichVuBus = new DichVuBUS();
                    ArrayList<DichVuDTO> dSDichVu = dichVuBus.getAllDichVu();
                ...
                // Tự động tạo hóa đơn
                boolean hoaDonThanhCong = taoHoaDonTuDong(maDatPhong, maKhachHang, txtTongTienDatPhong);
                
                // Hiển thị thông báo kết quả
                if (allRoomsAdded && allServicesAdded && hoaDonThanhCong) {
                    JOptionPane.showMessageDialog(dialog, "Đặt phòng thành công và đã tạo hóa đơn!");
                } else if (!allRoomsAdded) {
                    JOptionPane.showMessageDialog(dialog, 
                        "Đặt phòng không hoàn chỉnh: Một số phòng không được thêm vào danh sách!",
                        "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                } else if (!allServicesAdded) {
                    JOptionPane.showMessageDialog(dialog, 
                        "Đặt phòng không hoàn chỉnh: Một số dịch vụ không được thêm vào!",
                        "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                } else if (!hoaDonThanhCong) {
                    JOptionPane.showMessageDialog(dialog, 
                        "Đặt phòng thành công nhưng không thể tạo hóa đơn tự động!",
                        "Cảnh báo", JOptionPane.WARNING_MESSAGE);
                }
                ...
                JOptionPane.showMessageDialog(dialog, "Đặt phòng thất bại!", 
                    "Lỗi", JOptionPane.ERROR_MESSAGE);
            }
        } catch (Exception ex) {
            JOptionPane.showMessageDialog(dialog, "Lỗi khi đặt phòng: " + ex.getMessage(), 
                "Lỗi", JOptionPane.ERROR_MESSAGE);
            ex.printStackTrace();
        }
    }

    private void themDichVuVaoPhieu(JDialog dialog, 
                                JTable dichVuTable, 
                                DefaultTableModel dichVuTableModel,
                                JTextField txtSoLuong, 
                                DefaultTableModel suDungDichVuTableModel,
                                JTextField txtTongTienDichVu,
                                JTextField txtTongTienPhong,
                                JTextField txtTongTienDatPhong,
                                JTextField txtMaDatPhong) {
    ...
    private void xoaDichVu(JDialog dialog, 
                        JTable suDungDichVuTable, 
                        DefaultTableModel suDungDichVuTableModel,
                        JTextField txtTongTienDichVu,
                        JTextField txtTongTienPhong,
                        JTextField txtTongTienDatPhong) {
    ...
    private void suaSoLuongDichVu(JDialog dialog, 
                                JTable suDungDichVuTable, 
                                DefaultTableModel suDungDichVuTableModel,
                                DefaultTableModel dichVuTableModel,
                                JTextField txtTongTienDichVu,
    ...
    private void themPhong(JDialog dialog, 
                        JTable phongTable, 
                        DefaultTableModel phongTableModel,
                        DefaultTableModel phongDatTableModel,
                        JSpinner spinnerNgayNhan,
                        JSpinner spinnerNgayTra,
                        JTextField txtTongTienPhong,
        ...
        private void xoaPhongDaDat(JDialog dialog, 
                            JTable phongDatTable, 
                            DefaultTableModel phongDatTableModel,
                            DefaultTableModel phongTableModel,
                            JTextField txtTongTienPhong,
                            JTextField txtTongTienDichVu,
    ...
    private void capNhatDatPhong(JDialog dialog, 
                            JTextField txtMaDatPhong, 
                            JTextField txtSDT,
                            DefaultTableModel phongDatTableModel, 
                            JSpinner spinnerNgayNhan, 
        ...
        try {
            ...
            DatPhongDTO datPhongMoi = new DatPhongDTO(maDatPhong, maPhongChinh, maKH, ngayNhanChung, ngayTraChung, 0);
            ...
            if (datPhongBUS.updateDatPhong(datPhongMoi)) {
                ...
                HoaDonDTO hoaDon = hoaDonBUS.getHoaDonByMaDatPhong(maDatPhong);
                if (hoaDon != null) {
                    hoaDon.setMaDatPhong(maDatPhong);

                    // Cập nhật tổng tiền mới nếu muốn
                    int tongTien = 0;
                    try {
                        String tongTienStr = txtTongTienDatPhong.getText().replaceAll("[^0-9]", "");
                        if (!tongTienStr.isEmpty()) {
                            tongTien = Integer.parseInt(tongTienStr);
                        }
                    } catch (NumberFormatException e) {
                        tongTien = (int) hoaDon.getTongTien();
                    }
                    hoaDon.setTongTien(tongTien);

                    // Nếu muốn cập nhật ngày tạo hóa đơn (hoặc giữ nguyên)
    ...
    private boolean taoHoaDonTuDong(String maDatPhong, String maKhachHang, JTextField txtTongTienDatPhong) {
        try {
            HoaDonBUS hoaDonBUS = new HoaDonBUS();
            String maHoaDon = hoaDonBUS.tangMaHoaDon();

            // Lấy mã nhân viên đang đăng nhập từ TaiKhoanBUS
            String maNhanVien = BUS.TaiKhoanBUS.getMaNVDangDN();
            if (maNhanVien == null || maNhanVien.trim().isEmpty()) {
                JOptionPane.showMessageDialog(null, "Không xác định được mã nhân viên đang đăng nhập. Vui lòng đăng nhập lại!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return false;
            }
            ...
            try {
                ...
                System.err.println("Lỗi khi chuyển đổi tổng tiền: " + e.getMessage());
            }
            ...
            return hoaDonBUS.themHoaDon(hoaDon);

        } catch (Exception e) {
            System.err.println("Lỗi khi tạo hóa đơn tự động: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
